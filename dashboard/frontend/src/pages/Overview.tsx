...
export function Overview() {
  ...
  const load = useCallback(async () => {
    try {
      const [k, a, w] = await Promise.all([
        api.get('/api/overview/kpis').catch(() => null),
        api.get('/api/alerts/active').catch(() => ({ alerts: [] })),
        api.get('/api/incidents/wordcloud').catch(() => ({ words: [] })),
      ]);
      if (k) setKpis(k);
      setAlerts(a?.alerts ?? []);
      setWords(w?.words ?? []);

      // Build chart from recent windows or predictions
      const recent = await api.get('/api/overview/recent-windows').catch(() => null);
      if (recent?.windows?.length) {
        setChart(recent.windows.slice(-24).map((w: any) => ({
  ...
  /* KPI values */
  const totalConsumption = kpis?.total_consumption_kwh ?? kpis?.totalConsumption ?? 79_675;
  const activeZones      = kpis?.active_zones ?? kpis?.activeZones ?? 312;
  const totalMeters      = kpis?.total_meters ?? kpis?.totalMeters ?? 4_365;
  const totalIncidents   = kpis?.total_incidents ?? kpis?.totalIncidents ?? 231_841;
  const activeAlerts     = alerts.length ?? kpis?.active_alerts ?? 8;
    ...
    <div style={{ animation: 'fade-in 0.4s ease-out both' }}>
      ...
      <div style={{ padding: '24px 0 0' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 16, marginBottom: 24 }}>
          <KpiCard icon={Zap}   iconBg="rgba(124,58,237,0.2)"  label="Total Consumption (kWh)" value={totalConsumption.toLocaleString()} badge={2.4}  sub="This month" />
          <KpiCard icon={Users} iconBg="rgba(59,130,246,0.2)"  label="Active Zones"   value={activeZones.toLocaleString()}      badge={4.7}  sub="Daily" />
          <KpiCard icon={Activity} iconBg="rgba(34,197,94,0.2)" label="Total Meters"   value={totalMeters.toLocaleString()}      badge={1.2}  sub="This week" />
          <KpiCard icon={Eye}   iconBg="rgba(245,158,11,0.2)"  label="Total Incidents" value={totalIncidents.toLocaleString()}   badge={-0.8} sub="Worldwide" />
              ...
              {(['A','B','C','D']).map((zone, i) => {
                ...
                return (
                  <div key={zone} style={{
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>Zone {zone}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
          ...
          <div className="card-atonist" style={{ padding: '20px' }}>
            ...
            {[
              { label: 'meters_aggregated_15min', pct: 97 },
              { label: 'incidents_enriched',      pct: 84 },
              { label: 'ml_predictions',          pct: 92 },
              { label: 'feedback_nlp',            pct: 78 },
              { label: 'data_quality_metrics',    pct: 100 },
              { label: 'dashboard_alerts',        pct: 95 },
            ].map(({ label, pct }) => (
