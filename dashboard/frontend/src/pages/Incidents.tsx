    ...
    const load = async () => {
      try {
        const [w, inc] = await Promise.all([
          api.get('/api/incidents/wordcloud').catch(() => ({ words: DEMO_WORDS })),
          api.get('/api/incidents').catch(() => ({ incidents: DEMO_INCIDENTS })),
        ]);
        if (w?.words?.length) setWords(w.words);
        if (inc?.incidents?.length) setIncidents(inc.incidents);
      } catch {}
    };
    load();
